...
import { PackageForPlatformUseCase } from "../PackageForPlatformUseCase.use-case.js";
...
describe("PackageForPlatformUseCase", () => {
  test("rejects an unknown platform id before touching the store", async () => {
    ...
    const result = await new PackageForPlatformUseCase(store).execute({
      campaignId: "camp",
      assets: [asset()],
      platforms: ["myspace"],
    });
  ...
  test("writes an empty-items manifest when no asset matches the platform ratio", async () => {
    ...
    expect(result.value.platforms[0].manifestPath).toBe("camp/platforms/instagram-feed/manifest.json");
    expect(store.writeManifest).toHaveBeenCalledTimes(1);
