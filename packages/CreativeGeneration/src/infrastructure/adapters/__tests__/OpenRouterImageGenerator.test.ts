...
describe("OpenRouterImageGenerator", () => {
  test("returns an openrouter-sourced, cover-fitted image and maps the aspect ratio", async () => {
    ...
    const out = await new OpenRouterImageGenerator({ apiKey: "k" }).resolveBackground(
      product,
      ratio("16:9"),
      ctx,
    );
  ...
  test("reads an image from images[].url when image_url is absent", async () => {
    fetchMock.mockResolvedValueOnce(
      res({ json: messageWith({ images: [{ url: pngDataUrl() }] }) }),
    );
    const out = await new OpenRouterImageGenerator({ apiKey: "k" }).resolveBackground(
      product,
      ratio(),
  ...
  test("honours a custom model id", async () => {
    ...
    await new OpenRouterImageGenerator({ apiKey: "k", model: "x-ai/custom" }).resolveBackground(
      product,
      ratio(),
      ctx,
    );
  ...
  test("falls back when fetch rejects with a non-Error reason", async () => {
    ...
    const out = await new OpenRouterImageGenerator({
      apiKey: "k",
      fallback: fallback(),
  ...
  test("retries with image+text modalities on a modality-mismatch error", async () => {
    fetchMock
      .mockResolvedValueOnce(
        res({ ok: false, status: 400, text: "unsupported modalities for this model" }),
      )
      .mockResolvedValueOnce(
        res({ json: messageWith({ images: [{ image_url: { url: pngDataUrl() } }] }) }),
      );
  ...
  test("falls back on a non-modality HTTP error", async () => {
    ...
    const fb = fallback();
    const out = await new OpenRouterImageGenerator({ apiKey: "k", fallback: fb }).resolveBackground(
      product,
      ratio(),
      ctx,
    );
