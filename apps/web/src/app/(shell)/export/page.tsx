...
export default function ExportPage() {
  ...
  // X14 fix2: the picker offers what packaging will actually build — once the
  // reviewer has decided anything, only the approved set is sent, so the shapes
  // are derived from it. Approve statics and reject the html rows and no html
  // profile stays offered with nothing left for it to package.
  const packaged = hasDecisions ? approved : assets;
  const hasMotion = assets.some((a) => a.format === "motion");
  // Formats present in the run — an asset with no `format` key is a static one.
  const runFormats = new Set(packaged.map((a) => a.format ?? "static"));
