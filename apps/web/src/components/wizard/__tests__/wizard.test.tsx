...
describe("Wizard", () => {
  ...
  test("validates products, copy, output, and policy", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText(messages.products(1, "Randomized"))).toBeTruthy();
    ...
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText(messages.productColor)).toBeTruthy();
    expect(screen.getByText(messages.productIdDuplicate("solo"))).toBeTruthy();
    await user.clear(screen.getAllByLabelText("Primary Colour")[0]);
    ...
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText(messages.minDistance(6))).toBeTruthy();
    await user.clear(screen.getByLabelText("Min distance"));
    ...
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText(messages.layout)).toBeTruthy();
    expect(screen.getByText(messages.tone)).toBeTruthy();
    expect(screen.getByText(messages.background)).toBeTruthy();
    expect(screen.getByText(messages.paletteShift)).toBeTruthy();
    await user.click(screen.getByRole("button", { name: "headline-top" }));
    await user.click(screen.getByRole("button", { name: "bold" }));
    await user.click(screen.getByRole("button", { name: "procedural" }));
    await user.click(screen.getByRole("button", { name: "0" }));
    ...
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText(messages.platforms)).toBeTruthy();
    await user.click(screen.getByLabelText("linkedin"));
    await user.click(screen.getByRole("button", { name: "Next" }));
    expect(screen.getByText("Review")).toBeTruthy();
  });
