...
/**
 * The incomplete sections as ids, so the button hands `revealSection` the id —
 * never a label that would have to be lowered back into one (the old map did,
 * and "Variation Policy" is not an id). The labels come from `SECTION_TITLES`,
 * the one vocabulary.
 */
function getIncompleteSections(state: EditorState): SectionId[] {
...
export function StatusLine({
  ...
  const sentence = useMemo((): { lead: string; sections: SectionId[]; tail: string } => {
