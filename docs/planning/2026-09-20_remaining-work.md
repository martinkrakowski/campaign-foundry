...
| **RW-D4** | **A blocked lane is not scheduled.** Four lanes below wait on an unanswered question, and this plan does not guess an answer to make a wave look full.                                                                                                                                                                                                                                                     | RW-12 (`RunRegistryPort`) and the whole §6 deferral list wait on **D64**; RW-9 (multi-window drafts) waits on **D82**; RW-7 (beat-boundary drag) waits on **D138**; RW-14 (SE0) is a presentation question two documents answer differently. They appear in §5 with their blocker named, not in a wave.                                                            |
...
| **L1** | Low          | **Bookkeeping has accumulated into its own lane.** M4's unapplied amendments, PD1/PD2/PD4, the §42 id-collision grep, the D-id index, V3's stranded fence. Individually trivial, collectively a day, and every one of them is a thing a future reader will trust and be wrong about.                                                                                                                                                                                                                                     |
...
| **RW-2**  | **Real job progress.** `done`/`total` are `0/0` while running and jump to `n/n` at the end, so progress is binary.                                                                        | `fs-job-store.ts`, `jobs.ts`, `jobs.get.ts`, the web poller          | `2026-08-25` §10         | 0.5 d            |
| **RW-3**  | **4:5 aspect ratio.** Absent since August; `RATIO_VALUES` is still the three social ratios.                                                                                               | `aspect-ratios.ts`, compositor, **goldens**                          | `2026-08-25` §10 Q2      | 0.5 d            |
| **RW-4**  | **Per-layer frames (L10).** Canvas-relative position/size per layer, defaults byte-identical.                                                                                             | `creative-geometry.ts`, `brief-template.ts`, compositor, **goldens** | `2026-09-08` D130        | 2 d              |
| **RW-5**  | **The `fill` kind and generative region (L11 / VF3).**                                                                                                                                    | `brief-template.ts`, compositor, **goldens**                         | `2026-09-08` D131/D132   | 2 d              |
...
| **RW-8**  | **Canvas handles (SE5)** — drag a layer's frame on the preview. **Also needs the preview-footprint API lane, which does not exist.**                                                      | `CreativePreview.tsx`, preview route                                 | `2026-09-16` SE5 + §0 C3 | 2 d + unscoped   |
...
| **RW-18** | **Bookkeeping** — M4's amendments, PD1/PD2/PD4, the §42 collision grep, the D-id index, V3's fence.                                                                                       | `docs/`, `.claude/skills/`                                           | L1                       | 1 d              |
...
| 2    | **RW-4** frames (L10)                            | Takes the compositor lock for the whole wave. Longest chain in the tree (H1), so it starts now.           |
...
| Slot | Lane                      | Note                                                                               |
| ---- | ------------------------- | ---------------------------------------------------------------------------------- |
| 1    | **RW-5** fill (L11)       | Takes the compositor lock. **Serialised behind RW-4 by RW-D1**, not by dependency. |
