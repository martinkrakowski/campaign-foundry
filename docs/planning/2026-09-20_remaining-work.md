...
**Date:** 2026-09-20 · **Status:** **ALL FOUR WAVES CLOSED (2026-09-21).** Wave A: RW-11 (#542), RW-1 (#543), RW-4 (#544), RW-10 (#545), RW-7 (#546). Wave B: RW-2 (#547), RW-5 (#548) — the fill half only; D132 was then settled by amendment and shipped as **RW-26 (#549)**. Wave C: RW-18 (#550); **RW-24 needed no lane** (#499 fixed it on 2026-09-18, two days before this plan scheduled it) and **RW-3 is BLOCKED on Q2**. Wave D: RW-23 (#551), RW-15 (#552); **RW-22's thumbnails already shipped** (T-D1(c)) and its write path is behind **D64** by this plan's own C1. **Ten of the eleven dispatchable lanes shipped; the eleventh was already done.** What remains in this repository is gated on three questions — **D64** (the identity fork, C1; **answered 2026-09-24: (b) database-fronted**, so its lanes now wait on a storage plan), **Q2** (4:5; the fork and its 96-test measurement are on the RW-3 row) and **D82**.
...
| **RW-3** | **4:5 aspect ratio — BLOCKED on Q2, which is unanswered.** `randomized-campaigns-and-motion.md:380` still reads "**Q2** — When to add a 4:5 `AspectRatio`? … **A later Phase 5 extension only**", and that plan's status line names it as its one open question. Scheduling it here violated **RW-D4** for the third time, after RW-6 and RW-17. **Measured 2026-09-21:** adding `"4:5"` to `RATIO_VALUES` costs one compile error (the exhaustive label map) and **96 failing tests**, of which the decisive ones are the four in `policy-hash-stability.test.ts` — `VariationPolicy` resolves an absent ratio axis to `AspectRatio.all()`, and `aspectRatio` is a hashed field, so a fourth ratio **moves `policyHash` for every brief that does not pin `axes.ratio`** and the re-roll path refuses them all with "Plan changed since the last run". `GenerateCampaignUseCase` also renders `AspectRatio.all()` unconditionally, so every classic campaign gains a fourth cell. **The fork:** default-on (honest to Q2, needs a recorded hash migration and accepts that existing campaigns cannot be re-rolled) or vocabulary-only with the default selection unchanged (the conditional-spread idiom, nothing breaks, but classic briefs have no ratio selector so 4:5 is reachable only through a variation axis). | `aspect-ratios.ts`, compositor, **goldens**, nine web consumers, `messages.test.ts`'s raw-ratio gate, **and `VariationPolicy.vo.ts` + every policy-hash golden, which this row omitted** | same §10, Q2 | **blocked** |
...
| **RW-22** | **L7's remainder — HALF SHIPPED, HALF BLOCKED.** Template thumbnails already ship: `TemplateLibrary.tsx`'s `TemplateThumb` draws them from `layers` under recorded decision T-D1(c) (never fetched — a stored image needs a `thumbnail` field `CreativeTemplate` does not carry, and a compositor render would put a `/preview-frame` call behind every card). The **write path** is genuinely absent — `templates.get.ts` and `[ref].get.ts` are the only routes — but it is behind **D64**: this plan's own C1 names "the D62 template library" among the work that fork gates. | template library | `creative-templates-and-units.md` status line | **blocked (D64)** |
...
| **RW-24** | ~~Slots M1 — the mixed static+motion ceiling overcount.~~ **ALREADY SHIPPED — #499 (`f711a836`), 2026-09-18**, two days before this plan scheduled it. Both halves are closed and tested: the domain pins `axisProductSize` at 32 with `not.toBe(48)`, the editor pins 96 with `not.toBe(24 * 3 * 2)`. The M1 row in `creatives-as-slots.md` was never restamped, and this plan copied it forward. **The inventory method is why:** `merge-base --is-ancestor` over cited PRs proves what shipped and cannot see a closure it did not cite — §5 says as much about omissions, and the same hole admits stale-open rows. | — | `creatives-as-slots.md` | **done** |
...
| Slot | Lane                    | Note                                                                                                                               |
| ---- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 1    | ~~**RW-3** 4:5~~ | **Blocked on Q2 (RW-D4).** Nine web consumers plus the raw-ratio gate — and the policy-hash goldens this plan did not count. |
| 2    | ~~**RW-24** slots ceiling~~ | **Already shipped (#499).** Not a lane.                                                                                            |
...
| 3    | ~~**RW-22** L7 remainder~~ | **Thumbnails shipped (T-D1(c)); write path behind D64.** Not a lane. |
