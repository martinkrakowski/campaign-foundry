...
| Lane | Task | Buys |
|---|---|---|
| **T1** | **Server-rendered preview frame** (**D52**). An API route renders one cell through the real compositor at a requested ratio and returns it; the editor shows that image. Cache by brief-hash + ratio + treatment. | "Reflect the layout exactly" becomes true, permanently. Retires C1–C5 as a class rather than one at a time. |
