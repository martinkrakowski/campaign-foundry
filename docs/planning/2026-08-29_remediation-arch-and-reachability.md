...
| **D6** | ~~Descriptor provenance survives a reload.~~ **WITHDRAWN — it already does.** What remains is narrower and worth doing anyway: `PersistedAsset`'s *type* omits a field that is genuinely persisted, and nothing pins the round trip. | Complete the type; add a regression test. **Do not make `isPersistedAsset` stricter** — rejecting a row for a malformed descriptor would drop the whole asset, a behaviour change in service of a defect that does not exist. |
...
| R2.3 | **Complete the `PersistedAsset` type** — it describes a persisted row and omits `descriptor`, which every row has carried since descriptors existed. Type only: **do not add validation that can reject a row.** A malformed descriptor must not cost the whole asset | `apps/api/server/lib/report.ts` |
