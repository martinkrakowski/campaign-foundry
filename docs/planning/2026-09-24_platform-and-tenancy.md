...
| **PT-7a-metering** | **Admission is `generate.post.ts`** (`acquireJob`, `:156-174`), not `pipeline.ts`, which runs after the 202. The quota check goes there. **Metering:** each provider adapter is wrapped at construction in `pipeline.ts` (`imageGenerator()`, `:66-112`: Gemini, OpenRouter, Firefly), and copy generation (the `OpenRouterCopyGenerator` construction in `copy.post.ts`) is wrapped too, since D175 counts every generation. A cached result (`cached: true`) is not metered. Each call writes a `usage` row: org, provider, model, units, key owner (platform or org). **Quota** is a column on `org` (0007, additive: `alter table org add column …`), counted per calendar month. **Refusal** is 429 with `code: "quota_exceeded"`, before any provider call. **With `STORE_BACKEND=fs`** the usage port is a no-op and the local operator is unmetered; only the Postgres backend can admit another org at all. Metering lives in `apps/api` wrappers, never in `packages/*` (D167). | `0007_usage` | new usage port and adapter and their tests; `pipeline.ts` (construction only); `generate.post.ts` (the admission region only); `copy.post.ts` (the construction line only) | read env for quotas; store or read org keys (PT-7b) |
