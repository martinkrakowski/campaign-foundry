...
- **Still open:** **the auth provider** (D174b; blocks PT-1), **D176** (recommended: a `KeySealerPort` with a
  host-secret KEK now, KMS later; blocks PT-7b), and **the hosting platform** for the API and workers,
  which D176's KMS and the network paths to Aiven and Backblaze depend on.
