...
**Date:** 2026-09-24 · **Status:** **r2 — D166, D167, D173, D174a and D175 STAMPED (owner, 2026-09-24); Phase 0 done: **PT-0a done** — [#569](https://github.com/martinkrakowski/campaign-foundry/pull/569) (`b3322320`), [#570](https://github.com/martinkrakowski/campaign-foundry/pull/570) (`a48fcc9d`), [#571](https://github.com/martinkrakowski/campaign-foundry/pull/571) (`ca7a9520`). **PT-0b1 done** — [#572](https://github.com/martinkrakowski/campaign-foundry/pull/572) (`2f64f8fb`), [#573](https://github.com/martinkrakowski/campaign-foundry/pull/573) (`f9234f99`). **PT-0c done** — [#574](https://github.com/martinkrakowski/campaign-foundry/pull/574) (`27ed7d52`). **PT-0b2 done** — [#575](https://github.com/martinkrakowski/campaign-foundry/pull/575) (`bfbac87f`), [#576](https://github.com/martinkrakowski/campaign-foundry/pull/576) (`1df9fdbe`); **DoD 1 met.** **PT-0d done** — [#577](https://github.com/martinkrakowski/campaign-foundry/pull/577) (`df975038`), [#578](https://github.com/martinkrakowski/campaign-foundry/pull/578) (`0ff44ae9`); `cf:decisions` retired (DoD 4 still waits on PT-5's keys). **Phase 0 closed, PT-0e included (2026-09-24).** **D166, D174a (PostgreSQL on Aiven) and D175 (amended: BYOK in) STAMPED 2026-09-24.** **PT-3 foundation:** [#579](https://github.com/martinkrakowski/campaign-foundry/pull/579) (`f0c420c4`), [#580](https://github.com/martinkrakowski/campaign-foundry/pull/580) (`29b839a6`) (decisions on Postgres behind `STORE_BACKEND=postgres`). D174b (Better Auth; Google and email, email through Resend), D174c (Backblaze B2), D174d (Kafka on Aiven) and D176 (host-secret KEK behind a port; hosting undecided) STAMPED 2026-09-24/25. **Wave `platform-and-tenancy-w01` closed 2026-09-25 (§4.1):** PT-3c [#584](https://github.com/martinkrakowski/campaign-foundry/pull/584) (`7a2a27c1`), PT-3d [#585](https://github.com/martinkrakowski/campaign-foundry/pull/585) (`548c30b5`), PT-3e [#582](https://github.com/martinkrakowski/campaign-foundry/pull/582) (`e43be164`), PT-6a [#586](https://github.com/martinkrakowski/campaign-foundry/pull/586) (`e15a6c21`), PT-7a [#587](https://github.com/martinkrakowski/campaign-foundry/pull/587) (`a783422e`), PT-1a [#589](https://github.com/martinkrakowski/campaign-foundry/pull/589) (`d1e5678d`); follow-ups [#583](https://github.com/martinkrakowski/campaign-foundry/pull/583) (`99e54517`), [#588](https://github.com/martinkrakowski/campaign-foundry/pull/588) (`fee8568f`), [#590](https://github.com/martinkrakowski/campaign-foundry/pull/590) (`77fb0d72`). D168, D169 and D171 STAMPED 2026-09-25 (kafkajs chosen for PT-6b); D170 and D172 remain proposed. **Wave w02 closed 2026-09-26 (§4.2/§4.3):** PT-6a2 [#594](https://github.com/martinkrakowski/campaign-foundry/pull/594) (`b569d087`), PT-7a2 [#591](https://github.com/martinkrakowski/campaign-foundry/pull/591) (`b86cc0f5`), PT-1b1 [#592](https://github.com/martinkrakowski/campaign-foundry/pull/592) (`cfbd309d`), FU-encode-tmpdir-leak [#593](https://github.com/martinkrakowski/campaign-foundry/pull/593) (`d024f150`). **w03 (§4.3):** PT-1b2, FU-server-job-awareness, PT-2a.**
...
**Shipped 2026-09-25, all eight lanes, in migration order:** PT-3c #584 (`7a2a27c1`), PT-3d #585 (`548c30b5`),
PT-3e #582 (`e43be164`), PT-6a #586 (`e15a6c21`), PT-7a #587 (`a783422e`), PT-1a #589 (`d1e5678d`);
FU-mid-run-verdicts #583 (`99e54517`), FU-pools-race-test #588 (`fee8568f`); plus #590 (`77fb0d72`), a test
that closed two l11 mutations surviving since #584. Staging stays on `STORE_BACKEND=fs` until PT-8. The wave
record is in `.agents/session-log.md`.
...
### 4.2 Wave `platform-and-tenancy-w02` (defined 2026-09-25, after w01 closed)

The owner asked for the remediation work and the remaining waves in the recommended order, with agy
`gemini-3.8-flash-high` implementing every lane. D168, D169 and D171 were stamped the same day. **Only one
lane adds a migration (PT-7a2, `0009`).** Every lane inherits §4.1's rules.

| Lane | Delivers | Migration | Owns | Must not |
|---|---|---|---|---|
| **PT-6a2-run-fence** | **The run id fences report and decision writes (D171, D78).** Today a run whose lease lapsed (or whose deadline already failed it) still writes its report: `generate.post.ts:223` calls `writeReport` after the work, and `FsJobStore.completeJob`/`failJob` (`fs-job-store.ts:229-268`) only check that the entry exists. **Enumerated:** (1) `ReportStorePort.writeReport` and `DecisionStorePort.writeDecisions` take an optional `fence?: { runId: string }`, documented on the port; no fence means today's behaviour (the decisions PUT and the CLI pass none). (2) `PgReportStore` and `PgDecisionStore` check the fence **inside their existing write transaction**: `select 1 from job where id = $run and org_id = $org and campaign_id = $campaign and status = 'running' and lease_expires_at > now() for share`, and throw `JobLeaseLostError` (`pg-job-store.ts:25`) when no row returns, before any upsert. (3) The fs path: `lib/report.ts` `writeReport` and `lib/decisions.ts` `retireDecisions` take the fence and, before the store write, refuse unless the job store entry for `runId` (the port's `getStoredJob`, reached through `getJobStore(scope)`) is `running`. This is not atomic, which is acceptable on the single-process fs backend; say so in a comment. (4) `FsJobStore.completeJob` and `failJob` refuse (throw `JobLeaseLostError`) when the entry is no longer `running`, matching the pg fence; `runJob` must still settle the slot. (5) `generate.post.ts` passes `{ runId: jobId }` to its `writeReport` call (that line only). (6) `PgJobStore.getRunningJobId` (`:179-185`) adds `lease_expires_at > now()`, so a lapsed, unreaped row no longer reads as running. **Tests, one per item:** a pg report write and a pg decisions write refused for a lapsed lease (pre-seeded job row), and accepted for a live one; the fs late write refused after `failJob`; fs `completeJob` after `failJob` refused; `getRunningJobId` ignores a lapsed row; no fence leaves behaviour unchanged. **Re-anchor** any of `l7`, `l11`, `l12`, `pt-0a-report-store`, `pt-0d-decision-store`, `pt-3b-pg-decisions`, `pt-3c-reports-on-postgres`, `pt-6a-job-lease-rows`, `rw-2`, `r5` whose anchor moves. | none | both ports, the four report/decision adapters, `fs-job-store.ts`, `pg-job-store.ts` (`getRunningJobId` only), `lib/report.ts`, `lib/decisions.ts`, `generate.post.ts` (the `writeReport` line only), their tests | change `decisions.put.ts` or the CLI; touch the admission region of `generate.post.ts` |
| **PT-7a2-quota-reservation** | **A quota that holds under concurrency (D175).** Today `assertUnderQuota` (`metering.ts:49-54`) reads the quota and this month's count, and a usage row is written only after the provider returns (`:125`, `:164`). Parallel cells and concurrent jobs all pass against the same stale count. **Enumerated:** (1) `0009_usage_reservation.sql`: `usage` gains `status text not null default 'recorded' check (status in ('reserved','recorded'))`; reserved rows carry `created_at` as today. (2) `UsageStorePort` gains `reserve(): Promise<string \| null>` (a reservation id, or null at quota), `settle(id, record: UsageRecord)` (the existing `UsageRecord` type) and `release(id)`. (3) `PgUsageStore.reserve` runs in one transaction: `pg_advisory_xact_lock(hashtext('usage:' \|\| org))` (precedent `pg-job-store.ts:164-176`), read the quota (a missing org means 0, as now), count this month's rows that are `recorded` or `reserved` within `RESERVATION_TTL_MS` (a named constant, 1 hour), insert a `reserved` row or return null. `settle` updates that row to `recorded` with its fields; `release` deletes it. (4) `FsUsageStore`: `reserve` always returns an id, and `settle`/`release` are no-ops (the local operator stays unmetered). (5) `MeteredImageGenerator` and `MeteredCopyGenerator` become reserve → call → settle, with `release` in a `finally` on failure, a cached result or a fallback-owned result; null from `reserve` throws `QuotaExceededError`. (6) The admission check at `generate.post.ts:154-162` stays as a best-effort pre-check. **Tests:** reserve at quota returns null; a released reservation frees its slot; a reservation older than the TTL no longer counts; settle turns it into a recorded row with the fields; the fs store never refuses; each wrapper releases on failure; and a `TEST_DATABASE_URL`-gated test where two concurrent `reserve` calls against quota − 1 admit exactly one (the #586 pattern). **Re-anchor** `pt-7a-metering` if the admission line moves. | `0009_usage_reservation` | `usage-store.port.ts`, `pg-usage-store.ts`, `fs-usage-store.ts`, `metering.ts`, `0009`, `generate.post.ts` (admission region only), their tests | touch the `writeReport` line of `generate.post.ts`; add a dependency |
| **PT-1b1-auth-api-surface** | **What the sign-in UI (PT-1b2) needs from the API.** **Enumerated:** (1) `GET /campaigns/capabilities` (already allowlisted, `tenant.ts:15`) adds `auth: { mode: "local" \| "better-auth", google: boolean }`, where `google` is true only when both Google settings are present (the same condition `instance.ts:81-83` uses). (2) `memberTenant` (`lib/auth/membership.ts:16-20`) honours the session's `activeOrganizationId` when the user is a member of that org, and otherwise falls back to the first membership by `org_id` as today. A non-member active org falls back; it never errors. (3) No web changes. **Tests:** capabilities under each mode, with Google on and off; an active org the user belongs to wins; a non-member active org falls back; no active org falls back. | none | `capabilities.get.ts`, `lib/capabilities.ts`, `lib/auth/membership.ts`, `middleware/tenant.ts` only if the session shape requires it, their tests | touch `apps/web`; add a dependency |
| **FU-encode-tmpdir-leak** | **Stop the suite filling `$TMPDIR`** (2,867 dirs, 9.9 GB on 2026-09-25). **Enumerated:** (1) `capability-race.test.ts` `afterEach` (`:115-125`): the directory removal, the env restore and `resetJobs` run in a `finally` around the settle wait, so a settle timeout (`:96`) can no longer skip them. (2) `CanvasFfmpegVideoCompositor.ts:201-206`: `mkdtemp` moves inside the `try` whose `finally` releases `encodeGate`, so a failing `mkdtemp` cannot leak a gate slot. (3) Demonstrate before and after: count the `cf-caprace-*` and `cf-encode-*` dirs in `$TMPDIR` around one run of the capability-race file. **Tests:** a settle timeout still removes the dir; a failing `mkdtemp` releases the gate. | none | `capability-race.test.ts`, `CanvasFfmpegVideoCompositor.ts` and its test | raise any `testTimeout` or `hookTimeout` |

**Shared, declared:** `generate.post.ts`: PT-6a2 owns the `writeReport` line and PT-7a2 the admission region.

**Moved out, and why:** resolving the storage scope once per request in the pools routes (a w01 follow-up)
goes into **PT-2**, which rewrites every port call site for ownership anyway. The w01 cross-test write came from
a request that started late, after the next test had changed `PROJECT_ROOT`, not from a mid-request re-read
(`projectRoot()` memoises per module instance). `87a4ea93` already removed the cold start that delayed it.

**Later waves, in order:**
- **w03:** PT-1b2 (sign-in page, header user menu and org switcher; `better-auth` joins `apps/web`; a 401 or
  403 routes to sign-in); FU-server-job-awareness (a `GET /campaigns/jobs?campaignId=` route over the now
  lease-aware `getRunningJobId`; `run-context.tsx` adopts a running job after a reload); and PT-2
  (authorisation at the port, with the scope resolved once per request). PT-2 owns `lib/ports/*` alone in
  that wave.
- **w04:** PT-7b (org provider keys, after PT-2) and PT-6b (Kafka delivery with kafkajs, after PT-6a2).
  PT-5 needs its own split first, because it is the largest web change in the plan.
- **Owner-gated:** PT-4 (B2 bucket, scoped key, and a live `If-None-Match` check), then PT-8 and PT-9.

### 4.3 Wave `platform-and-tenancy-w03` (defined 2026-09-26, after w02 closed)

**Wave w02 shipped 2026-09-26, all four lanes:** PT-6a2 [#594](https://github.com/martinkrakowski/campaign-foundry/pull/594) (`b569d087`), PT-7a2 [#591](https://github.com/martinkrakowski/campaign-foundry/pull/591) (`b86cc0f5`), PT-1b1 [#592](https://github.com/martinkrakowski/campaign-foundry/pull/592) (`cfbd309d`), FU-encode-tmpdir-leak [#593](https://github.com/martinkrakowski/campaign-foundry/pull/593) (`d024f150`). Review changed two lanes materially: PT-7a2 gained a reservation hand-off through nested provider fallbacks (without it, a fallback at quota − 1 aborted the run) and a check that a recorded row carries every field; PT-6a2's fs fence now holds the job's lock across the check and the write. The record is in `.agents/session-log.md`.

**PT-2 settled by the owner, 2026-09-25** (four questions from the PT-2 survey against `ccf12dfe`):
- **Team scope (D166):** a campaign with no team (`team_id` null) is visible to the whole org. A member with teams sees
  the org-wide campaigns plus their teams' campaigns, and the org's `owner` and `admin` roles see everything. Reports,
  pools and decisions inherit visibility through their campaign; they gain no team column. `campaign.team_id` becomes
  `text references team (id)` (it is `uuid` with no foreign key today, `0004_brief.sql:11-12`, while `team.id` is
  `text`).
- **Platform admin and `audit_event`: deferred.** No cross-org access exists, so PT-2 ships no admin path. The table
  lands with the first feature that needs cross-org access.
- **404 policy:** an id route answers 404 when the campaign is not the caller's, including `GET /campaigns/result`,
  `GET /campaigns/decisions` and the assets list, which answer 200 with an empty body today. `generate`, `plan` and
  `preview-frame` require `brief.id` to be a stored campaign of the caller's org.
- **Teams are Postgres-only.** The fs backend stays single-operator; Better Auth already requires Postgres.

PT-2 splits into three PRs (w03 → w04): **PT-2a** the cross-tenant route harness, the scope resolved once per request
in the eight routes that re-resolve it, and 404 tests for the id-taking reads; **PT-2b** ownership at the port for
writes and body-driven routes, with `package.post.ts` moved onto the output port; **PT-2c** team scope (the
`campaign.team_id` migration and filters in `pg-brief-store.ts`).

| Lane | Delivers | Migration | Owns | Must not |
|---|---|---|---|---|
| **PT-1b2-sign-in-ui** | **Sign-in and org switching in the web app (D174b, PT-1).** Today the web app has no auth UI, and a 401 or 403 surfaces as "Pipeline API unreachable (HTTP n)… yarn dev" (`run-context.tsx:155-160`) or a `BriefsApiError`. **Enumerated:** (1) `better-auth` joins `apps/web` at the version `apps/api` pins (`^1.7.6`), and the regenerated `yarn.lock` travels with the PR. A `lib/auth-client.ts` calls `createAuthClient` (from `better-auth/react`) with the `organizationClient` and `magicLinkClient` plugins (from `better-auth/client/plugins`) and `basePath: "/api/auth"`, same-origin through the existing rewrite (`next.config.ts:38`). (2) A sign-in route `(auth)/sign-in/page.tsx`, outside the `(shell)` layout: an email field that sends the magic link, and a "Continue with Google" button shown only when capabilities report `auth.google`. (3) `getCapabilities` (`briefs-api.ts:54-84`) keeps the new `auth` field (it currently rebuilds the object from `motion`, `reason` and `version`). (4) `Header.tsx`: under `auth.mode === "better-auth"`, a user menu (email, sign out) and, when the user has more than one org, an org switcher that calls `organization.setActive` and reloads, beside `<ThemeToggle />` (`:145`) and in `MobileMenu` (`:184`). Under `local`, the header is unchanged. (5) A 401 `{code:"unauthenticated"}` from any pipeline call routes to `/sign-in`, and a 403 `{code:"no_membership"}` shows a "no organisation yet" state, not the pipeline-unreachable error. **Tests:** each item, including the header unchanged under `local` and the Google button hidden when `auth.google` is false. | none | `apps/web/package.json`, `yarn.lock`, `apps/web/src/lib/auth-client.ts`, the sign-in route, `briefs-api.ts` `getCapabilities`, `Header.tsx`, the 401/403 handling in `run-context.tsx` and `briefs-api.ts`, their tests | touch `apps/api`; add any other dependency |
| **FU-server-job-awareness** | **A reload during a run knows the run is still going.** Today `loading` (`run-context.tsx:652`) is true only in `execute` and the re-roll, and the job id lives in the `execute` closure, so after a reload the verdicts and Package are enabled mid-run (#583's pause only covers this tab's own run). **Enumerated:** (1) `GET /campaigns/jobs?campaignId=` (a new `routes/campaigns/jobs/index.get.ts`, id validated with `SAFE_ID_PATTERN`) returns `{ jobId }` from `getRunningJobId` (lease-aware since PT-6a2), or 404 when no run holds the campaign. (2) `run-context.tsx`, after restoring a brief (the mount effect ≈:810-850, and `setBrief` ≈:725-782), calls the route. If a run holds the campaign, it sets `loading`, calls `beginRun()` and polls and commits exactly as `execute` does (≈:972-1000). (3) A 404, or a network failure, leaves the page as today. **Tests:** the route's found, not-found and invalid-id cases; the reload-adopts-a-running-job path; the no-job path unchanged. | none | the new jobs route and its test, `run-context.tsx` (the restore path only) and its tests | change `lib/jobs.ts` or any store |
| **PT-2a-tenant-harness-and-reads** | **The first third of PT-2 (§4.3's PT-2 decisions).** **Enumerated:** (1) A shared test harness `routes/__tests__/tenant-harness.ts` that mounts a route behind a middleware setting `event.context.tenant` (the pattern in `lib/__tests__/tenant.test.ts:8-21`), for both backends: fs, with a temporary `PROJECT_ROOT`/`OUTPUT_DIR`, org `local` at the root and `acme` under `orgs/acme`; and Postgres, with `migratedDatabase()`, `STORE_BACKEND=postgres` and a `database()` mock. (2) The storage scope is resolved once per request in the eight routes that re-resolve it: `briefs/[id]/duplicate.post.ts` (11 calls), `briefs.post.ts` (6), `pools/copy.post.ts` (6), `pools/[briefId].patch.ts` (4), `package.post.ts` (2), `briefs/[id].put.ts` (2), `assets.post.ts` (2), `assets.get.ts` (2). (3) One cross-tenant test per id-taking read, proving it answers 404 for another org's id: `GET assets`, `GET jobs/[id]`, both `packages` routes, `GET pools/[briefId]` and `GET output/[...path]`, on both backends where the route has a pg path. Where a route does not already answer 404, fix the route. (`GET result` and `GET decisions` move to PT-2b: they must tell "not the caller's campaign" (404) apart from "the caller's, no run yet" (200 empty), and the web client, which throws on any non-OK status (`run-context.tsx` `fetchPersistedRun` and `fetchDecisions`), must read that 404 as "none" in the same PR.) | none | the eight routes, the read routes listed in (3), the harness, their tests | touch `lib/ports/*` (PT-2b); change a write route's behaviour beyond resolving the scope once |
