...
**Date:** 2026-09-24 · **Status:** **r2 — D167 and D173 STAMPED (owner, 2026-09-24); Phase 0 started: PT-0a part 1 merged as [#569](https://github.com/martinkrakowski/campaign-foundry/pull/569) (`b3322320`).** D166, D168 – D172, D174 and D175 remain proposed.
...
|---|---|---|---|
| **PT-0a** | **Part 1 merged (#569):** reports through `ReportStorePort`, the latest pointer retired, and a corrupt report now rejects instead of reading as absent (review finding). **Part 2:** output reads and the brief-files path wrappers. **Everything persists through a port (C2).** A `ReportStorePort` (reports and `package.post.ts`'s report read; the global latest pointer retires, and `result.get.ts` requires a `campaignId`). An `OutputStorePort` (renders, the packages listing and zip). The dead path helpers are deleted. File adapters only, no behaviour change beyond the retired pointer. | `lib/report.ts`, `result.get.ts`, `package.post.ts`, `packages/[campaignId].get.ts`, `packages/[campaignId]/[platformZip].get.ts`, `lib/brief-files.ts`, `lib/asset-files.ts`, new ports | — |
