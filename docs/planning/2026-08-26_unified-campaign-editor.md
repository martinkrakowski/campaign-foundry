...
**Date:** 2026-08-26
**Status:** Revised v2.1 (decisions D1–D14 locked after an Architect review and a Reviewer pass; Q1/Q2 resolved)
...
| **D5** | **Field parity with the brief schema.** Every `CampaignBrief` field is editable: id, region, audience, message, localized message, products (id, name, colour, logo upload/path, `inputAsset`), **treatments** (classic; not editable today), variation policy (count, seed, minDistance, coverage, axes incl. `headline` pool, and — new — `motion`/`duration`), output (formats incl. `motion`, platforms filtered to compatible ones). | The remaining plan-deferred item "wizard has no motion controls" closes here. |
...
| **D7** | **Client validation mirrors `parseBrief`; the API stays authoritative.** Reuse `validate.ts`; inline errors per field; the sticky bar shows the first blocking error; Save/Apply disabled while invalid. | No duplicate rule engine — one module, already tested. |
...
| **D13** | **Stale-write protection.** Before Save & apply on a loaded brief the editor re-fetches that brief by id and compares it to `savedSnapshot`; a mismatch (another tab or an external edit) opens a conflict dialog (reload theirs / overwrite). A `storage` event from another tab's save on the same `cf:draft` key marks this tab's draft stale. The selector list is refreshed on mount, after Save, and on window focus. | Two tabs cannot silently overwrite each other; the client-side D9 check is best-effort and the API 409 remains the backstop (stated). |
| **D14** | **Navigation guard mechanism.** Next App Router has no route-change event, so shell links (header tabs, sidebar Edit/Create new, picker) go through one `useGuardedNavigate()` hook that consults the editor's dirty state and opens the D8 dialog; `beforeunload` covers tab close/reload only. | D8 is implementable as specified; no `window.confirm`; the mechanism is decided before E1, not during. |
...
│ status: 🟢 saved & applied · 🟡 applied, unsaved edits · 🔴 draft     │
│         not applied — plus the first blocking error, linked to its    │
│         section                                                        │
└──────────────────────────────────────────────────────────────────────┘
```
...
- `GET /campaigns/capabilities` → `{ motion, reason? }` (new, read-only, 10 lines + test).
- No other API change: create/replace/duplicate/upload/plan/pools routes already exist.
...
| E1.3 | Selector data: `listBriefs()` on mount, after Save, and on window focus (D13); **New brief…** resets; picker select, picker **Create new**, and sidebar **Create new** all go to `/brief` and dispatch the same `load` **from E1 onward** (the wizard is no longer an entry point, only a route until E3). D8 dirty guard via `useGuardedNavigate()` (D14) + `beforeunload`. `briefs-api.updateBrief()` (PUT) restored. | `components/shell/{BriefPicker,Sidebar,Header}.tsx`, `lib/briefs-api.ts`, `lib/use-guarded-navigate.ts` |
| E1.4 | D9: id read-only for loaded briefs; **Save as…** copy dialog stating the original stays; overwrite confirm for new-id collisions (client list + 409 backstop) shared by Save and Save as…; D3 **Save & apply** semantics (PUT for the loaded id, POST for new); D13 pre-save re-fetch + conflict dialog + `storage` staleness. | action bar, `editor-state.ts`, `briefs-api.ts` |
...
| # | Task | File(s) |
|---|------|---------|
| E2.1 | `GET /campaigns/capabilities` route + test. | `apps/api/server/routes/campaigns/capabilities.get.ts` |
...
| E2.3 | Motion controls: `formats` (motion gated on capability with reason), `motion` kinds, `duration` list; `minDistance` bound from active axes (shared rule with the VO); Output platforms filtered by compatibility (mirror of `validateFormatPlatformCompatibility`). **D12:** capability off + loaded motion brief → controls read-only, fields preserved verbatim through `toBrief()`/Save; Apply shows the parser's 400. | `editor-state.ts`, `validate.ts`, Output section |
| E2.4 | Tests: capability off → controls disabled with reason **and a loaded motion brief round-trips through Save unchanged**; compatibility filtering; estimate frames/minutes for motion; pool axis gating; drawer race guard; split view keeps the error strip. | `__tests__/` |
...
4. Switching briefs or following any shell link with unsaved edits prompts; discarding restores the selected brief; a loaded brief cannot be renamed in place — Save as… copies and says the original remains; landing on an existing id prompts before overwriting; a stale file (other tab) prompts before overwriting; toggling mode and back loses nothing; a reload after a crash offers the draft back only when it differs.
6. A motion brief opened on a machine without ffmpeg round-trips through Save with its motion fields intact.
