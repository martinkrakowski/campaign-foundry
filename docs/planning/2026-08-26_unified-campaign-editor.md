...
|----|----------|-------------|
| **D1** | ✅ **Landed, amended.** **One editor, one route.** `/brief` is *the* campaign editor for both new and existing briefs; the step wizard is removed. **Amendment (#92):** blank creation is `/brief/new`, not `/brief`. A blank draft is pristine, so it passes every dirty guard, and the adopt-the-active-brief effect put the previous campaign straight back — the intent has to belong to the route to survive longer than the click that expressed it. `/new` stays as a redirect to `/brief/new` so wizard-era bookmarks land somewhere. | The sidebar's **Edit** lands on `/brief`; **Create new** lands on `/brief/new`. The `New campaign` header tab from #62 is removed. |
