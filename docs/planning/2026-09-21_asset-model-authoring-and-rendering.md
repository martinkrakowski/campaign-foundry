...
**Status:** **CLOSED except AR6 — all six decisions stamped (D161 after the fact, 2026-09-24).** AR1 (#554), AR2 (#555), AR3 (#556), AR4 (#557) and AR7 (#558) merged; AR5 was closed by #555. The DoD gaps a 2026-09-23 verification found — the doubled headline, the html profile never packaged end to end, the retired kind refused without its fix, the click-target control on every kind — were closed by `2026-09-23_asset-model-follow-up.md`: AF1 (#562), AF2 (#563), AF3 (#559), AF4 (#564), AF5 (#560), AF6. **AR6 (`<video>` in the bundle) is blocked on the storage plan.** D64 was answered (b), database-fronted, on 2026-09-24, and that plan has not been written. **Third draft**, revised the same day on the owner's corrections: D160 no longer mints a
...
**Verified against:** `main` at `d7e7cb0f` (drafted); status re-verified at `cd9abd4e`. Every claim below cites code that was read, not recalled.
...
| **AR7** | **`image-html.required` still carries the dead `html` layer kind.** AR2 retired what that kind draws and compiles (`drawHtml` is a no-op, `assembleHtml` emits nothing for it, `LAYER_PROPS.html = []`) but left it required on `CREATIVE_TYPE_RULES["image-html"]`, so every display-ad brief is stuck with a layer it can never disable and the canonical template has no default `static-text` — a fresh HTML ad ships with no headline. Found red-teaming AR4; not fixed there because the files are outside AR4's Owns column and retiring a layer kind is its own migration (existing briefs already carry the required `html` layer — see AR5's class of problem). **Goldens confirmed unmoved**: the compositor's golden tests build a `CompositeRequest` by hand (`NodeCanvasCompositor.goldens.test.ts`), never through `CANONICAL_TEMPLATES`. | `creative-types.ts`, `creative-templates.ts`, `load-brief.ts` (existing-brief compatibility) | **merged** (#558) |

**AR5's own finding.** Re-reading AR2 (#555) while dispatching AR7: its boundary refusal for the retired `elements` field already reads *"is no longer a layer field; author the copy as a static-text layer"* (`load-brief.ts:334`) — that names the fix, which is exactly AR5's stated deliverable ("rewritten to layers, **or refused at the boundary with a message naming the fix**"). AR5 reads as already done by #555 rather than a lane still to dispatch. Recorded as a finding for the owner to confirm, not treated as a unilateral closure here.
...
- **It does not schedule anything.** AR1 (#554), AR2 (#555), AR3 (#556), AR4 (#557) and AR7 (#558)
  are merged. AR5 reads as already done by #555 — see AR5's own row — rather than a lane still to
  dispatch. AR6 stays named and blocked on **D64**; per RW-D4 a blocked lane does not occupy a slot.
...
**Update, 2026-09-24:** the owner stamped D161 after the fact. The row above now says so. The paragraph before this one is kept because it is the record of how the gap happened. The same thing then happened a second time: the follow-up plan's AF1 (#562) shipped before its decision, D164, was stamped. That plan records it the same way.
