...
They share a shape: each is a mechanism that is _correct where you look_ and wrong where nobody does.
...
was told to take — _if the setting is ignored, that is the finding_ — did not arise, and the target
...
**`premise W3` retired — the shared definition exists.** The fence counted sites declaring their own captured-round comparison and expected 2; `apps/web/src/lib/use-latest-only.ts` is now the one definition and both sites adopt it, so the count is 0 and the fence has flipped as designed. It was a good fence: its own comment records that an earlier draft counted files containing the substring `generation`, which a rename would have satisfied without the work being done.
