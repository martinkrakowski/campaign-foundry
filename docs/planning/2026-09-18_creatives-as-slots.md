...
**Date:** 2026-09-18
**Status:** **COMPLETE, and every decision stamped.** SL0–SL4 shipped (#496, #497, #500, #504, #506). SL-D2 stamped option (a); **SL-D1, D3, D4, D5, D6, D7 and D9 were stamped on 2026-09-19** — each was verified against `origin/main` at `fc7ab434` rather than inferred from the lanes having merged, and each cell names what carried it. **SL5 dropped by the owner and SL-D8 withdrawn** — the rename is not wanted. A later fix to SL-D4's draw budget landed in #515.
...
## 1. What the code did before this plan — measured, not recalled

> **Tense note, added 2026-09-19 with the stamps.** This section was written as
> "today" on 2026-09-18 and is kept as the measurement the plan argued from. Two
> of its three facts are now history: SL2 removed the compaction
> (`const index = accepted.length`), and SL1 gave occupancy somewhere to live in
> the brief. Re-read it as the before-picture, not as a description of `main`.
...
| ID        | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Status                                         |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **SL-D1** | **Identity is the slot (`productId` + `index`), not the axis tuple.** Recorded with §2's refutation so the coordinate idea is not re-proposed from scratch. `Variant.ts:10` already said so.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **ADOPTED — `Variant.ts:10` already said so**  |
...
| **SL-D3** | **A deleted index is never reused.** Monotonic allocation is the entire fix for recreate-on-delete; compaction (`const index = accepted.length`) **was** the bug, and SL2 removed it — the cursor now advances and never revisits a slot.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **SHIPPED — SL2 (#500)**                       |
| **SL-D4** | **Append draws against occupants** — adopted from CL-D10. `conflicts(a, b, minDistance)` (`PlanCapacity.ts:97`) already takes pairs; the use case planned a _fresh_ set instead. Appending one creative to eleven must distance against the eleven. **Shipped through the use case's own `meetsMinDistance(candidate, history, …)`, not by calling `conflicts`** — the draw distances against the allocation history, which is a superset of the live occupants. Two implementations of one rule, paired in the comment at `PlanVariationsUseCase.use-case.ts:368`; `conflicts` remains the exhaustive path's copy, so do not go looking for a call that is not there.                                                                                                      | **SHIPPED — SL2 (#500); budget fixed in #515** |
| **SL-D5** | **`count` becomes the recipe's target cardinality; occupancy is what exists.** They diverge the moment a slot is tombstoned, and the UI must show the second. A run generates the occupied slots, not `count` fresh draws.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **SHIPPED — SL2 (#500)**                       |
| **SL-D6** | **`minDistance` is floored at 1, in both sites** — `meetsMinDistance` **and** `conflicts`. Independent of identity; it is SG-D7's finding and closes a live hole where the draw samples with replacement (83 of 120 distinct, operator-reachable through the Advanced stepper's `min={0}` with no warning). Flooring only `meetsMinDistance` leaves the suite **green** while distinct collapses to 1 of 120 — measured.                                                                                                                                                                                                                                                                                                                                                    | **SHIPPED — SL0 (#496), both sites**           |
| **SL-D7** | **`minDistance` is demoted to a generator control** — it governs append and re-roll against occupants, and is not a runtime invariant over an editable list. A global Hamming floor over rows the operator can edit fights the operator.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **ADOPTED — demotion verified; see SL-D8**     |
...
| **SL-D9** | **Seeds stay index-derived.** Moving them buys "same tuple ⇒ same background" at the price of changing every existing variation PNG, and does not deliver add/delete.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **HELD — seeds unchanged**                     |
...
| **M1** | ~~Medium~~ **CLOSED** | **A mixed `static`+`motion` brief overcounts its own ceiling** — `VariationPolicy.vo.ts` and `editor-state.ts` multiplied the motion factor across every ratio while `enumerateAxes` only enumerates over `motionRatios` (48 against a real space of 32). **Fixed by #499 (`f711a836`) on 2026-09-18**, both halves: the domain pins `axisProductSize` at 32 with an explicit `not.toBe(48)` (`mixed-format-ceiling.test.ts`), and the editor pins 96 with `not.toBe(24 * 3 * 2)` (`ceiling-parity.test.ts`). **This row stayed marked open for two days**, and on 2026-09-20 the scheduling plan copied it forward as lane RW-24 — a lane against a gap that no longer existed. The inventory method that missed it (`merge-base --is-ancestor` over cited PRs) proves what shipped and cannot see a closure it did not cite; the orchestration skill now carries that check. |
