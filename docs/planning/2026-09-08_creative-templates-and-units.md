...
| **D134** | **The initial per-layer props are overrides of the geometry each layer already reads — nothing invented.** Added 2026-09-08, because L3's brief could not be written: the plan said layers carry `props` and typed them `LayerPropsFor<kind>`, but never said what any kind's props *are*. A carrier with no contents is a field nothing uses. The vocabulary is therefore taken from what the compositor reads per layer today: `shade: { alpha? }` (**withdrawn by R-D4, 2026-09-15** — the tone axis is never absent, so it would always silence it); `accent: { solidHeight?, fadeHeight? }`; `logo: { width?, margin? }`; `static-text` / `animated-text`: `{ anchor?, typeFloor? }`. Every prop is **optional**, and **absent means the value the layer resolves today** — from `CREATIVE_GEOMETRY`, the treatment, or the style, exactly as now. `image`, `video`, `html` and `fill` carry no props in L3 — theirs arrive with the lanes that draw them (D131, D132) or with the format family (D122). | Three properties make this the right first vocabulary. It is **grounded**: every entry is a value some layer already reads, so nothing is speculative. It is **inert on arrival**: all props absent means every render is byte-identical, so the goldens hold and L3 is provably a carrier lane. And it is **the shape D130 needs**: a frame is another optional per-layer override resolved against a default, so L10 extends this mechanism rather than replacing it. What it deliberately is **not** is a second home for `style` or `Treatment` — D126 keeps those until their own deprecation lane, and duplicating them here would create exactly the two-sources-of-truth defect this arc exists to remove. |

| **D135** | **A reorder that would hide or mute a layer warns; it never refuses.** Moving a layer is checked against an **occlusion table** — declared data beside the compatibility table, not logic scattered through the editor — that says which kinds obscure which and how badly: `image` and `fill` are **opaque** (anything below them is hidden), `shade` and `accent` are **attenuating** (text below them is muted, not lost), and `logo` is **local** (it overlaps only where it sits). The check runs **per layer on every reposition**, so it names the specific pair — *"the shade layer now sits above the headline and will mute it"* — never a generic "this may look wrong". **Warnings are advisory: the user may proceed.** A required-kind rule (D129) still refuses; occlusion does not. | The owner's ask, 2026-09-09. **Warn, not refuse**, because designers put type under a scrim deliberately and a system that forbids it is wrong more often than the user is — the same reasoning §9 already applies to the contrast check. **A table, not conditionals**, because the moment occlusion logic lives in the editor it disagrees with the compositor, which is the drift D121 exists to remove. And **per pair, not per move**, because a warning a user cannot act on gets dismissed by reflex; naming the two layers makes it a decision instead of noise. |
| **D136** | **Occlusion findings join the existing compliance surface; they do not get a new one.** `CompliancePort` already declares `validateLegalCopy` and `validateBrandColorDensity` returning `ComplianceResult { passed, score?, reason? }`, and `/compliance` already renders that vocabulary. A layer-order finding is a third member of that family — **advisory** severity — surfaced in the editor at the moment of the move **and** aggregated on the compliance page with the others. | The owner asked for this to be *attuned to the branding and integrity checks*, and those checks already exist with a port, a result type and a page. Adding a parallel warning channel would give the product two places that answer "is this creative sound?", which is exactly the two-sources-of-truth defect this arc has spent eleven lanes removing. The brand-density floor already depends on the accent band being visible — **an occlusion that buries the accent band is a brand-compliance fact, not a cosmetic one**, and that is the strongest argument for one surface rather than two. |
...
| **L3** | *(shipped as **L3a** — required field + editor round-trip — and **L3b** — props.)* **Per-layer props on the brief** (F6, **D134**), **plus the two items L1b handed forward.** The nested `template` block: ordered layers, each with its own props; YAML round-trip and key order; validation per layer kind; draft normalisation. **`CampaignBrief.template` becomes required here** and the compiler sweep runs (D120 as amended). **`toBrief` must emit `template`** — it does not today, so an editor save drops it (Qodo, #263); harmless while templates are canonical-only and re-derived from the type, a data-loss defect the moment a user edits layers. | `CampaignBrief.ts`, `brief-yaml.ts`, `load-brief.ts`, `editor-state.ts`, tests | A template a brief can carry |
...
| **L7** | *(**Partly shipped.** The port, the filesystem adapter and the three canonical seeds shipped earlier with no consumer and no route. **L7a2** (dispatched under the id `TL1`, which collides with this
plan's sibling `2026-09-16_studio-editor.md` — see the note below) gave the port its first consumer: `GET /campaigns/templates` and `GET /campaigns/templates/:ref` — `:ref` is `id` or `id@version`, a request with no version returns the highest version present, an unknown version is a 404 never a different version of the same id, and a version that does not parse as a positive integer is a 400 naming the field. Still unshipped: thumbnails — two of the three seeds need `html`/`video` drawers that do not exist — the library page, and any write path.)* **The template library** (D123). `TemplateStorePort` mirroring `BriefStorePort`, a filesystem adapter over versioned files, `GET /templates` and `GET /templates/:id@:version`, the three canonical templates as its seed, thumbnails rendered by the real compositor. Ownerless, so **independent of D64**. | new port + adapter, routes, `campaign/library/*`, tests | The product the owner described |
| **L8** | **Reorder** (D128, **D135**, **D136**). Move within the array; `above`/`below` constraints in the compatibility table; the preview follows. **Plus the occlusion guard**: a declared table of which kinds obscure which, checked per layer on every reposition, emitting an **advisory** `ComplianceResult` that names the pair — and surfaced through the existing compliance channel, not a new one. Also carries the duplicate-React-key fix deferred from L2b. | `creative-types.ts`, `derive.ts`, the layer-list control, `CompliancePort`/`ComplianceResult`, tests | Rung one of the ladder, and creatives that cannot silently bury their own brand |
...
- **L3**: a template round-trips through YAML **and the editor** verbatim — the `output.sizes`
  lesson: a subset the user chose comes back the subset they chose; a pre-template draft normalises.
  **`fromBrief` → `toBrief` preserves `template`** (the assertion #263 lacked, and the mutation is
  deleting the field from `toBrief`'s return). **`CampaignBrief.template` is required** and
  `git grep 'template!'` returns nothing — the non-null assertions L1b's optionality forced are
  gone, not silenced.
