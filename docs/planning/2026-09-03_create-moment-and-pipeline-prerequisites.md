...
**Date:** 2026-09-03
**Status:** **SHIPPED — stamped 2026-09-20.** P1 (#184), P2 (#187), W1 (#185), W2 (#188), W3 (#186) are on `main`. **Open:** the whole §6 deferral list — server-side drafts, server-minted ids, ownership on the ports, the D62 template library, markets fan-out, presigned uploads, external ingestion, and ports for reports/packages/output — **all gated on D64, answered 2026-09-24 as (b) database-fronted; they now wait on a storage plan, not a decision**. _Was: "for review"._
...
| id | Decision | Why |
|---|---|---|
| **D64** | **STAMPED — Owner, 2026-09-24: (b) database-fronted.** C7 was stamped the same day: assets are keyed by a tenant-scoped immutable id. *Original row:* **the owner's call: the identity model.** *(a) Object-store-only:* the slug stays the storage key, as the recorded S3 shape has it (`.agents/architecture.md:87` maps `brief.id -> s3://bucket/briefs/<id>.yaml`; `:97` prefixes assets by `briefId`). Keys are immutable, a rename is copy-then-delete, uniqueness is bucket-global. *(b) Database-fronted:* a row with a surrogate id, a mutable display name, and the slug as a per-workspace unique column — what D5 was reaching for. **P1 and P2 are fork-independent; W1's dialog is, but its seed mechanism and W2's client-derived id are the current-store shape and are retired under (b)** — see §3's *Fork exposure*. The consequence table is in §1 (C1, C7, C12, C23, C24 — cloud tier). | Every finding about the sketch's first field, about rename, about assets keyed by slug, about drafts and about delete resolves differently on the two sides. The owner's own words ("server side database") lean (b); the repo's only recorded target is (a). Deciding this is the highest-leverage act in the migration and it is not a plan author's to take. |
...
| **C7** | **STAMPED — Owner, 2026-09-24: assets are keyed by a tenant-scoped immutable id, never by slug** (for example `org/<orgId>/campaign/<campaignId>/…`), so a rename never touches storage and no key names another tenant's content. The platform plan (`2026-09-24_platform-and-tenancy.md`) implements it. *Original finding:* [intent] **Rename orphans assets under the slug-keyed S3 shape even under (b)**, unless assets are re-keyed to the surrogate id — a second decision D64 does not make by itself. | self-verified: `architecture.md:97, 100` |
...
| **C12** | [code] **Under (a) the slug keys five stores** — brief, assets, pool, report, packages — and only the first two sit behind a port. **Corrected by review (grok):** the job map is keyed by a UUID with `campaignId` as a scanned *field* (`jobs.ts:59-62, 75-78`), not by the slug; and the rendered PNGs are keyed by product and ratio only, a sixth implicit store C18 names. | self-verified: `report.ts:21-23`; `jobs.ts:59-62`; grok verdict *partly* |
| **C23** | [code] **A create-time write freezes the typed name.** The save reducer flips the source to `kind: "file"` with `loadedId: state.briefId` (`editor-state.ts:1023-1025`); `IdentitySection` then renders the slug read-only (`:115, 138-141`). Under (a) that is permanent — the object key is immutable and there is no delete route — so a typo in the dialog's first field becomes a permanent object. Dissolves under (b). *(Cited by D64 and §5.1; the row was missing in v1 — caught by review, grok.)* | self-verified: `editor-state.ts:1023-1025`; `IdentitySection.tsx:115` |
...
**Decide D64 first, but do not wait for it.** Wave 1 is four lanes and a fifth, mostly correct on both
...
400). Then W2 (needs P2's route), then W3 (needs W1). Parallelism cap 3. Were P1 and P2 ever
overlapped they would collide on `apps/api/server/routes/campaigns/__tests__/briefs.test.ts`, which
holds both the malformed-body tests and the duplicate tests; the schedule above keeps them apart.

**Fork exposure — corrected by review (grok), and it contradicts v1's headline claim.** v1 said
"nothing in wave 1 assumes a side of D64" and that only `createCampaign()`'s body plus W3 were
retired under (b). That understated it:

| Lane | Under D64(b) |
|---|---|
| **P1**, **P2** | Fork-independent. A typed boundary and a portable pool store are wanted either way. |
| **W1**'s dialog, strings, guard wiring, Duplicate fix | Fork-independent. This is the expensive part and it survives. |
| **W1**'s seam body, `create-seed-context.tsx`, the editor's seed consumption, the `/brief/new` landing | **Retired.** The whole seed carrier exists only because there is no server row to land on. Under (b) Create mints a row and lands on `/brief/<serverId>`. |
| **W2**'s seam-derived `newId` | **Retired.** A client-chosen slug is exactly the (a) shape (`duplicate.post.ts:29-36` requires `newId` in the body). Under (b) the copy's id comes from the server. |
| **W3** | **Retired**, as v1 already said. |

That exposure is accepted deliberately: the alternative is to build nothing until D64 is decided,
and the retired surface is one context file plus its consumer. It is *not* the "nothing assumes a
side" claim v1 made, and the plan should not have been sold on it.
...
- **v1's headline claim was wrong: wave 1 does have fork exposure.** "Nothing in wave 1 assumes a
  side of D64" survived one review (gemini answered §5.5 with "no lane assumes a side") and was
  overturned by the other (grok), correctly: W1's seed carrier and W2's client-derived id are the
  current-store shape. §3 now carries a *Fork exposure* table instead of the claim. **This is the
  reviewers' one substantive disagreement, and the dissent was right** — the argument for running
  both.
- **v1 specified `go("copy")`, which does not compile:** `go` takes a number
  (`use-step-navigation.ts:35`). Three more W1 mechanics were wrong or missing — the unspent step
  baton, the `useState`-initializer hydration trap, and the absent `attempted`/`touched` reset.
  All corrected by review (grok). This is the class the plan warned about in W1's own preamble
  ("the lane's hardest mechanic, named here so it does not thrash") and the author still got four
  details wrong; the lane brief must be read against the code, not against this plan.
- **v1's P1 scope left the same crash open for a numeric scalar**, and named three of `parseBrief`'s
  six call sites. Corrected (grok).
- **v1's `PoolStorePort` omitted the two `node:fs` seams the pool routes import**, so the lane's own
  Definition of Done contradicted its port surface; it also missed that `copyPool` has to rewrite
  the pool's own `briefId`. Corrected (grok).
- **C12 overcounted the slug's reach** (the job map is keyed by a UUID) and **C23 was cited by D64
  without a row existing**. Both corrected (grok).
...
**grok-4.6 (high), 2026-09-03 — delivered after 84 minutes; verdict: approve-with-changes.** It was
written off as stalled at the 78-minute mark (34 seconds of cumulative CPU, zero bytes) and landed
anyway; the write-off was reported and is corrected here. Cloud tier: C1, C4, C7, C9, C11, C16, C17
**hold**, each with the line read; **C12 partly** (the job map is keyed by a UUID, not the slug).
Eight findings, all accepted, every one verified against the code before folding in:

| Severity | Section | Finding |
|---|---|---|
| high | §3 / D64 | **Wave 1 does have fork exposure** — the seed carrier and W2's client-derived id are the (a) shape. Overturns v1's headline claim. |
| high | W1 | Four seed mechanics wrong: `go` is numeric, the baton must not be double-spent, consumption must be an effect, `attempted`/`touched` need resetting. |
| high | P2 | The port surface omits `poolPath` and `isPoolDirSymlink`, contradicting the lane's DoD; `copyPool` must rewrite `CopyPool.briefId`; add `setPoolStore`; take `withPoolLock(newId)` too. |
| medium | P1 | `typeof === "string" \|\| v == null` is the right check; array-or-object leaves the numeric crash open. Three more `parseBrief` call sites. |
| medium | D64 | C23 cited with no row. |
| medium | D67 | The picker's Duplicate — **found independently of Qodo**, same site, same conclusion. |
| low | P1 | Name which routes inherit the 400 rather than asserting all six. |

**On the disagreement.** gemini answered "no lane assumes a side of D64"; grok showed that two do.
grok was right, and the plan's central selling point changed as a result. That is the second time
in this repo these two reviewers have disagreed decision-gradingly, and the second time the dissent
carried the decision — the case for running both even when the first comes back clean.
