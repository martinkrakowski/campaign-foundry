...
| **SG-D22** | **`Generate` requires validated AND applied.** SG-D15 flips the verb on any edit, but freshness alone is not sufficient: `execute()` runs the **committed** brief, so an operator who validates a clean draft *without Applying* and presses Generate would spend GenAI credits on the stale committed document — D35's *"which brief?"* question silently answered wrong rather than dissolved. The toolbar's `Generate` therefore renders only when the validation is fresh (`validatedState === state`) **and** the draft is applied (`BriefEditor.tsx:919`), or else it applies and runs as one gesture. Never a `Generate` whose target can differ from the document that was validated. |
