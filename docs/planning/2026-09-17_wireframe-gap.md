...
| **SG-D7** | **If `mode` is retired, is `count` at its ceiling *exhaustive*?** | (a) Verify and rely on it — at `count = axisProductSize` the draw enumerates the space, so no separate concept is needed. (b) If it merely samples to that size, add an explicit exhaustive path. | **(a), VERIFIED 2026-09-18 — conditional yes, with one silent hole to close. SG-D1 may proceed.** *History of this row: a first draft claimed `count` was capped at 48 and that "count = everything" was therefore inexpressible. **That was wrong.** `editor-state.ts:734` sets the ceiling to `axisProductSize(state)` and clamps `count` to it, so the ceiling **is** the cross-product size and the 48 in the owner's editor was simply their axis space. A second framing — "enumerates exactly once **or** samples to that size" — was a **false dichotomy**: the real answer is enumerate-under-conditions, and the conditions are the whole content of this row.* **Verified by test** (`PlanVariationsUseCase.use-case.test.ts`, `describe "at count = axisProductSize (SG-D7)"`, 3 assertions + 1 reachability, each shown red under mutation). **Mechanism:** `DISTANCE_AXES` names *every* axis `enumerateAxes` varies, so Hamming 0 ⟺ the same point in the space; at `minDistance >= 1` no duplicate can ever be accepted, so `axisProductSize` accepted variants out of a space of `axisProductSize` is necessarily **the whole space exactly once**. It is the exhaustive search at `PlanVariationsUseCase.use-case.ts:112-116` that delivers it, not the random draw — at 120 points the 3 × count budget cannot fill the space, so the fallback is the load-bearing path. `executeVariation` (`GenerateCampaignUseCase.use-case.ts:543`) passes `plan.variants` through untouched, so the planner's property is the pipeline's property. **Three conditions, one of which is a real hole:** **(i) `minDistance 0` silently samples.** `meetsMinDistance` admits Hamming 0, so the random draw fills every slot with replacement — measured **83 of 120 distinct** at the ceiling. It is operator-reachable with no warning: the Advanced stepper is `min={0}` (`PolicySection.tsx:421`) and `validate.ts:313` accepts 0. The default is 1 (`Auto (1)`), so the default path is safe. **Fix is a floor of 1 in both `meetsMinDistance` and `conflicts`** — either alone leaves the other search degenerate (flooring only `meetsMinDistance` sends `minDistance 0` into `exhaustiveAccept`, where `conflicts` is then always `false` and the greedy returns `count` copies of one point; measured, distinct collapses to exactly **1** of 120). **Raise as its own lane. (ii) Spaces above `EXHAUSTIVE_MAX_SPACE` (4096) cannot enumerate** — the fallback is skipped and the plan is **refused with a shortfall error**, loudly, never silently short. Operator-reachable and not exotic: 2 products × 3 ratios × 2 layouts × 2 tones × 3 background sources × 10 palette shifts × a 6-text copy pool = 4320 points; measured *"accepted 4099 of count 4320"*. So "give me every combination" has a hard ceiling of 4096 combinations. Its message also misreads above the limit — it says *"lower count to 4320"* when count **is** 4320. **(iii) A mixed `static`+`motion` brief overcounts its own ceiling.** Both `VariationPolicy.vo.ts:271` and `editor-state.ts:707` multiply the motion factor across *every* ratio, while `enumerateAxes` applies it only to `motionRatios` — so the slider max exceeds the real space (measured 48 vs 32) and count at the ceiling is **unplannable**, failing loudly with a shortfall. **Separate defect, own lane; the fix is per-ratio multiplication in both sites.** **Net for SG-D1:** *"give me every combination"* survives retiring `mode` for any brief under 4096 combinations at the default `minDistance`; conditions (ii) and (iii) fail loudly and so cannot silently drop the guarantee; condition (i) is the one silent drop and is a two-line fix the retirement does not depend on. |
