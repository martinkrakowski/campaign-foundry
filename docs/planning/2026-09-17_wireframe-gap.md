...
|---|---|---|---|
| **SG-D1** | **Create modal: two tiles or four?** | (a) Collapse to `Static` │ `Video/motion` as drawn. (b) Keep the four grouped tiles — they are newer than the wireframe and carry real preset differences (`paid-social` is 7 platforms, `display-ad` is a different canvas union). | **(b) Keep four.** The wireframe predates the grouping, and collapsing would destroy the display-ad preset's distinct canvas. The wireframe's *intent* — "pick a kind, fast" — is already served by the two group headings. |
