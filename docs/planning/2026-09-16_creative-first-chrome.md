...
| **CC6** | D144 | **Shipped in this branch.** Regrouped the dialog's existing four tiles under Static (`social-post`, `paid-social`, `display-ad`) and Video/motion (`short-video`) headings (`createFormatGroupStatic` and `createFormatGroupMotion` in `messages.ts`). Kept `role="group"` with `aria-label` matching each heading (rather than `<fieldset>`/`<legend>`) within the named `createTypeLabel` group, avoiding browser layout and legend styling quirks while ensuring explicit accessible names for screen readers. | All four types remain reachable; `applyPreset` still runs exactly once; the motion group's `mode: variation` still comes from the preset, not the dialog. Mutation verified (`display-ad` into motion caught). |
