...
| **VE-D12** | ~~Cue timing from per-cue PCM byte length.~~ **SUPERSEDED by VE-D13, 2026-09-20 — it was wrong, and wrong in a way that would have shipped.** `bytes / (rate × channels × width)` is FILE duration, and TTS clips routinely carry trailing silence, so every cue after the first drifts late and the error accumulates. Recorded rather than deleted: the vendor half of VE-Q5 it settled still stands (OpenRouter, `hexgrad/kokoro-82m`, `POST /api/v1/audio/speech`, on the existing `OPENROUTER_API_KEY`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Superseded                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **VE-D13** | **Cues bind to an `AudioTrack`, never to a TTS response.** VE-D9 was never buggy: it assumes the speech generator is also an ALIGNER, and those are different products. `/audio/speech` is a waveform generator. So the audio service emits one frozen object and the video engine consumes only that — how the numbers were obtained is the service's business. Required: `schema_version`, `track_id`, `language`, `script_text`, `spoken_text`, `audio`, `duration_s`, `words`, `phrases`, `source`, `confidence`. **Two clocks, and the distinction is the whole fix:** `duration_s` is file length INCLUDING the TTS pad and is for muxing only, while `speech_start_s`/`speech_end_s` run first-word-start to last-word-end. **Never bind a cue to `duration_s`** — that is exactly what VE-D12 did. Phrase intervals are **exact** (measured clip + policy gap); word intervals are exact only when `source` is `native` or `aligned`, otherwise `allocated`.                                                                          | Owner, 2026-09-20                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **VE-D14** | **Two texts, and cues index the spoken one.** `script_text` is what the author wrote; `spoken_text` is what TTS actually said. Word `char_*` offsets index `spoken_text`; `script_char_*` maps back for author-space selectors. **No fuzzy matching on the hot path.** This is not fussiness: `"$4.2B"` is spoken "four point two billion dollars", so a cue bound to author text matches nothing. Selector order: `word_index`/`word_range` → `spoken_span` → `script_span` → explicit `phrase_index`; a range's time is `words[start].start_s … words[end].end_s`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Owner, 2026-09-20                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **VE-D15** | **The producer is swappable and labelled.** `source` is one of `native` \| `chunked` \| `aligned` \| `estimated` \| `hybrid`, and the consumer is told what it is holding. Native speech marks short-circuit everything — one call, `source: "native"`, same schema — so a vendor that returns marks is a drop-in upgrade rather than a rewrite. Forced alignment of the KNOWN script is an optional upgrade that overwrites word times and sets `hybrid`; it is not a second ASR, cannot hallucinate a different sentence, and is not on the hot path. **VE-D9 must refuse word-locked motion when covering words are below `confidence 0.6` and use the parent phrase interval instead** — the degradation is in the producer, never in the interface.                                                                                                                                                                                                                                                                                      | Owner, 2026-09-20                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **VE-D10** | **At most 3 distinct per-beat backgrounds per timeline**, enforced as a timeline authoring rule in `CopyTimeline.vo.ts` beside `MAX_BEATS`, so the parser and the editor refuse the same thing (D3). (Refined by plan review: generated-vs-procedural is the _variant's_ background source, `variant.backgroundSource`, invisible to the timeline validator — so the cap counts distinct values regardless of source, and "a procedural scene spends nothing" is a statement the **estimate** makes, in VE5b2.) **VE5b2 — scenes are uploaded assets, so they spend nothing:** since VE5a a scene is an uploaded asset path, never a generated one, so no scene ever adds an AI image call; the estimate sentence states this, only when the plan's timeline names a background.                                                                                                                                                                                                                                                              | Owner, 2026-09-15 (VE-Q3). No credit budget exists today, and variation mode multiplies scene cost by `count × ratios`; 3 covers open, middle and close.                                                                                                                                                                                                                                                                                                                                                                       |
| **VE-D11** | **"Does not add video decoding" stands.** VE6 is deferred, not scheduled; revisit only after VE3b's AAC golden proves audio determinism. **VE3b1 proved it**: the pinned `ffmpeg-static` binary decodes the music bed's arbitrary input bytes deterministically — one process, two runs, byte-identical MP4 output (`CanvasFfmpegVideoCompositor.audio.test.ts`, darwin-arm64) — and the audio byte golden (`CanvasFfmpegVideoCompositor.audio-golden.test.ts`) pins that result across processes on darwin-arm64, with linux-x64 re-proved by CI's record-goldens workflow. This is audio decode, not video decode — VE6's gap (a pinned _video_ decoder) is unaffected and still deferred.                                                                                                                                                                                                                                                                                                                                                  | Owner, 2026-09-15 (VE-Q4). Decode is unpinned (the compositor pins encode and scale only), uploads are base64 JSON capped at 2 MiB, and goldens would need committed video fixtures. A poster-only slice (one still extracted at upload) is the recorded middle ground if footage becomes a requirement.                                                                                                                                                                                                                       |
...
| ID     | Sev      | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **C1** | Critical | **Licensing rules out adopting the reference project.** The free repo's `LICENSE.md` says MIT with a placeholder copyright while its README says commercial use requires the paid RVE licence _and_ a Remotion licence. The paid licence bans extracting components, reserves audits and names a £50,000-per-breach penalty. Remotion is source-available, free only up to three people, and counts both companies' headcount when an agency's client owns the project. → VE-D1. |
...
**and** whose `source`, `pool`, `appliedSnapshot` and `capabilities` equal the _current_ ones; a `save`
...
### VE4 — Voiceover and captions · VE-D9, VE-D12 · **unblocked 2026-09-20**

> **The phrase chunker, when the vendor returns bytes only (VE-D15's `chunked`).**
>
> `spoken_text` → tokenize → split → TTS each phrase → **trim silence** → measure
> speech duration → allocate words inside the phrase → insert gap → concat PCM →
> `AudioTrack`.
>
> **Split order:** sentence punctuation → `;`/`:` → comma/dash _only if the phrase
> already has ≥ 4 words_ → soft-break word (`and`, `after`, `when`, …) _only if the
> current phrase already has ≥ 8 words_ → `max_words=18` / `max_chars=140`. Soft breaks
> cut **before** the break word. The ≥ 4 guard is why _"The market opened lower after
> overnight futures sold off, and yields jumped."_ does **not** split at `after` — only
> four words precede it — but does split at the comma.
>
> **Intra-phrase allocation:** `0.35 × chars + 0.65 × syllables`, minus a pause budget
> for internal commas, clamped to `[0.07 s, 0.90 s]`, then scaled to fill the measured
> clip. Those words are `timing: "allocated"`, `confidence: 0.55`; phrase rows stay
> `timing: "exact"`. So phrase boundaries are measured truth and word times inside a
> phrase are honest estimates that say so — which is what lets VE-D9 fall back.
>
> **What the producer must not do:** no per-word TTS (prosody dies); no second ASR of
...
> for one to plan against, so this is arithmetic on assumptions, not a quote — every
> input is written down so the whole thing can be corrected by changing one line:
>
> - a voiceover script of **~40 words ≈ 240 characters**, roughly 15 s of speech
> - **kokoro-82m is an 82M-parameter open model**, the cheap end of hosted TTS
> - no published price on the model page as of 2026-09-20 — this is the gap
>
> $0.002 is deliberately an OVER-estimate: it is roughly an order of magnitude above
> what small open TTS models typically charge for 240 characters, so a plan built on it
> should not be surprised on the upside. At 12 creatives that is **~$0.024 a run** —
> negligible beside image generation, which is the point worth carrying into any
> budget conversation.
>
> **How to replace it, cheaply.** Every speech response carries an `X-Generation-Id`
> header. Make one real call, then read that generation's actual cost from OpenRouter's
> activity page and put the measured figure here with its date. One call settles it, and
> until someone makes that call this number is a guess wearing a decimal point.

| #   | Task                                                                                                                                                                                                            | File(s)                                                                        |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 1   | Voiceover as a second audio source mixed under/over the music bed.                                                                                                                                              | `CanvasFfmpegVideoCompositor.ts`                                               |
| 2   | Caption cues bound to `AudioTrack.words[]` by the VE-D14 selector order, never to `duration_s` (VE-D13). Word-locked motion refuses below confidence 0.6 and falls back to the parent phrase interval (VE-D15). | new value object beside `CopyTimeline.vo.ts`                                   |
...
> **Three things VE-D12 does not settle, and the lane must.**
>
> 1. **The voice is part of the cache key.** kokoro offers 35+ voices (`af_`, `am_`,
>    `bf_`, … ), and VE-D9 caches by _provider, model id and seed_. There is no seed on
>    this endpoint; there IS a voice, and two voices are two different audio files for
>    one script. Pick a default voice, put it in the key, and put it in the golden.
> 2. **Determinism is assumed, not measured.** A golden can only pin the audio if the
>    same voice and text give the same bytes. Probe that once before a golden depends
>    on it — if it does not hold, the golden pins the WebVTT and the durations, not the
>    audio.
> 3. **The encoder maps exactly one audio track today** (`-map 1:a:0`, and "exactly one
>    AAC track" is VE3b's acceptance). Voiceover over a music bed is a second source and
>    needs a real mix, not a second `-i`. That is task 1's actual work.
