...
| **VE-D12** | **VE-Q5 answered (owner, 2026-09-20): OpenRouter, `hexgrad/kokoro-82m`, via `POST /api/v1/audio/speech`.** No new vendor relationship — `OPENROUTER_API_KEY` already carries the copy and image generators. **Cue timing does NOT come from word timings**: that endpoint returns raw audio bytes and no timestamps at all (verified against the model page, 2026-09-20). Each cue is synthesised as its OWN request in **PCM** — kokoro's default format — and its duration is read from the byte length, `bytes / (sampleRate × channels × bytesPerSample)`. Cue starts are the running sum. Exact, deterministic, no aligner, no transcription call, no new dependency, and the same total characters so the same spend.                                                                                                                                                                                                                                                                                                                   | Owner, 2026-09-20                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 2   | Caption cues timed to the **speech**, not to beat weights — **per-cue synthesis, timed from each cue's own PCM byte length** (VE-D12; VE-D9 said "word timings", which the chosen endpoint does not return and WebVTT does not need). | new value object beside `CopyTimeline.vo.ts`                                   |
