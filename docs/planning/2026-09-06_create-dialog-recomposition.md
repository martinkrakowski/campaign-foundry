...
**Date:** 2026-09-06
**Author:** orchestrator
**Status:** **K1 SHIPPED (#199). W1 SUPERSEDED — do not dispatch it. W2(a) shipped; W2(b) still
open and still owner-opt-in.** The plan was accepted on 2026-09-06 and Wave 1 (K1) dispatched; §8
records the review pass. **W1's premise died the next day** and the plan was never amended — see
§9, added 2026-09-19. A lane dispatched against W1 as written would revert five shipped decisions.
...
for a _format_ (stills vs. motion cutdowns), a _template_ from a scrolling rail, _five macro
regions_ picked off an animated world map, and _assets_ dropped into a project bin — then shows a
...
- and one is a _name collision_: the mockup's "Format" is this app's **Mode** (Classic /
...
1. **The form — adopt it now.** Numbered `01 / 02 / 03` sections with an eyebrow, a heading and a
   hint; option tiles that carry a _picture, a name, a tag, a blurb and a meta line_ instead of the
...
previews and then closes the door: _"Loading indicators (`animate-spin`… `animate-pulse`…) are the
only other permitted loops. **Every other animation in the system is a one-shot on interaction.**"_
And DESIGN.md:209-213: _"A loop is never the only carrier of meaning"_ — each motion glyph renders a
static cue **as well as** the animated one, _"because a paused animation still says 'this is moving'
to anything reading the DOM"_. The mockup's `@media (prefers-reduced-motion)` block does exactly
...
`apps/web/tailwind.config.ts` only _extends_ the default theme, so stock `animate-ping` (literally
...
_Note what does not need to change._ The one-shot `check-pop` on the selected badge and
...
data: a user who picks _EMEA_ on a map and lands in an editor whose region says `EU` has been
...
- **Format** — `static` / `motion`, an _Output-step_ axis, and gated:
...
`components/campaign/ModePanel.tsx` where its domain imports are legal, and the _tile_ it renders
...
_A different and defensible case, left alone:_ seven kit files import **types and constants from
`@campaignfoundry/*` domain packages** (`ratio-frame`, `creative-glyph`, `platform-card`,
`swatch-chip`, `duration-strip`, `preview-layers`). Those are the _domain's_ vocabulary, not a
...
exactly as it does today. (b) rewrites two tests that assert _modality_, which is **D84**'s open
...
| Map hub ripple                                                    | _dropped with the map_ (F2)                                                                             | —                                                                                                                                                                     |
| Unselected tiles at `opacity:.55 saturate(.45)`, selected at full | **Kept** — it is a transition, not a loop, and `AxisCard` already inverts its preview tile on selection | This is the mockup's strongest single move: the preview _becomes_ legible when chosen.                                                                                |
...
3. **Every string is a prop** — never a literal in the kit and never a `messages.ts` import.
   `messages.ts` is the _feature's_ catalogue, and three kit files import it today
...
- **A mutation check per behavioural claim**: the PR body names the source mutation that makes each new test fail, and it was run. Specifically — and this repo has been bitten by both — a tile test must fail when `aria-pressed` is removed from `OptionTile`, and the boundary test must fail when a campaign import is added to a kit file. _A test that cannot fail against the defect it names is not done._
...
- PR body carries a _Deviations_ section, even if empty.
...
- **W2 (a)**: a dirty Cancel does not close; typed answers survive _Keep editing_; _Discard and close_ leaves no `cf:create-seed`; an empty draft still closes on the first Cancel; **Escape-then-Escape returns to editing with the answers intact** — Escape never destroys, and _Discard and close_ is the only thing that does.
...
1. **D90 — is a confirm-on-close wanted at all?** It is the one _new_ behaviour here, and it is
   W2(a). Declining it costs nothing else: K1 and W1 ship unchanged and the dialog closes exactly as
   it does today. _The plan recommends taking it_, because the dialog is about to get long enough
...
2. **D89(b) — does the resume two-way move inline?** Separate question, separate price: it rewrites
   ...
   _The plan recommends deferring it until D84 lands_ — F4 is a cosmetic double-scrim today, not a
...
5. **The kit's padding contradicts DESIGN.md and nobody has said which is right.** DESIGN.md:197
   says _"dialogs `p-6`"_; `DialogHead` and `DialogFoot` both use `px-4 py-3`
...
- **"The mockup is a restyle."** It is not. Six of its affordances are new _product surface_ (§5),
...
- **"`packages/ui` doesn't exist here."** It does, under a different path: `apps/web/src/components/ui`
  ...
  that points at it. The reusability problem is not the _location_; it is that five kit files import
...
- **"The resume two-way can just move inline."** The plan's first draft folded it into D89 as
  presentation. It is not: two tests assert it is a _separate labelled dialog_ and that Escape
...
- **"Adopt the mockup's tokens."** The mockup's `--rgb-*` triples and its single border colour are
  an _earlier_ version of this repo's token system. `tokens.css` has since split the border token in
...
_(to be completed by the two-reviewer pass)_
...
## 9. W1 is superseded — recorded 2026-09-19, after a red-team before dispatch

W1 was about to be dispatched. Red-teaming it against `origin/main` at `5ef91fb1` — the step that
exists precisely to catch this — found that **every section it would build has been overtaken**, and
that the status line above was the only thing still presenting it as live.

W1 recomposes the dialog into three numbered sections over **five answers**. The dialog asks **two**:
campaign name (`CreateCampaignDialog.tsx:350`) and campaign type (`:368`). Its own docstring says
what happened:

> Region, audience, the map, the start-from rail, the mode panel, **the numbered sections and the
> jump strip** all left with **S1/T3** — they already live in Identity or the editor sidebar.

| W1 would build                                          | what the tree says now                                                                                                                             |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| three numbered `SectionBlock`s + `JumpStrip` footer     | removed by S1/T3 (`2026-09-07_two-field-create.md`, all five lanes landed)                                                                         |
| `01 · Targeting` — region `ChipGroup`, audience `Input` | moved to Identity by S1/T3                                                                                                                         |
| `02 · Start from` — the picker in its own section       | left the dialog with S1/T3                                                                                                                         |
| `03 · Mode` — `ModePanel` on `OptionTile`               | **D99/D110: the dialog never sets mode.** D144 (`2026-09-16_creative-first-chrome.md`) regrouped the type tiles by format instead, shipped as #455 |

`2026-09-07_two-field-create.md` names **D86** — this plan's question-set decision — in its
`Relates to` line, so the supersession was deliberate and a day old when it happened. It was simply
never written down here.

**What a dispatch would have cost.** W1 owns its own tests, so a lane would have rewritten the
assertions that pin the two-field dialog and reported green while reverting S1, T3, D99, D110 and
D144. That is the exact shape of "a lane ships a plan the product moved past", and the only thing
standing between this plan and that outcome was reading the code first.

**What survives, and is NOT superseded:**

- **K1** — shipped as #199. The four primitives live in `packages/ui/src/` (`option-tile`,
  `section-block`, `jump-strip`, `guard-bar`) with `kit-boundaries.test.ts`; the kit later moved out
  of `apps/web` under D92's own lane. `OptionTile` is what the dialog's type tiles render today.
- **W2(a)** — the inline discard guard: shipped. `guardOpen` state and `GuardBar` in
  `CreateCampaignDialog.tsx`, commented `W2(a) (D90)`.
- **W2(b)** — the resume two-way moved inline: **still open, still owner-opt-in.** It is independent
  of the question set, and the condition it targets is unchanged — the resume prompt is still a
  second `role="dialog"` (`:462`), which is F22's two-scrims shape and D84's subject. The plan's
  original framing stands: it moves only if the owner says so, because it rewrites two tests that
  pin modality.

**If a recomposition is still wanted**, it is a new plan against the two-field dialog, not this one.
`SectionBlock` and `JumpStrip` exist and are unused by the dialog, which is a fair prompt for that
conversation — but two fields may simply not want numbered sections.
